...
### Added
- Integrated `FlexibleGridLayout` as a native uLayout container
- Added responsive row/column modes, cell-axis fitting, grid alignment, child margins, and floating-child exclusion
- Added EditMode regression tests for grid layout and floating sizing
- Added `IuLayoutContainer` so nested layout types share one parent notification seam
- Added an Inspector warning for circular parent `FitContent` and child `Grow`/`Percent` size rules

### Fixed
- Prevented invalid grid dimensions and divide-by-zero results
- Moved grid placement from the measurement pass to Unity's layout set passes
- Prevented floating expansion from accumulating across rebuilds
- Made runtime property changes mark their parent layout for rebuild
- Made layouts skip non-`RectTransform`, inactive, disabled, floating, and standard `LayoutElement.ignoreLayout` children safely
- Made empty `FitContent` layouts retain their padding size
- Updated stale references to removed and undocumented features
- Prevented `SpaceBetween` from producing negative gaps when content overflows
