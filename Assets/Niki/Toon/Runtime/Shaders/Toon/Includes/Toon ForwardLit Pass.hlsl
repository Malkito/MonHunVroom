...
// TAM density uses only the hatch data exported by CupOHappiness's lighting seam. Direct
// toon response is always present; indirect light and occlusion are explicit,
// independent material opt-ins. Specular, emission, and rim are excluded.
half CupOHappinessTAMResolveTone(CupOHappinessHatchData tamHatchData)
{
    half tone = saturate(tamHatchData.directDiffuseTone
        + tamHatchData.indirectDiffuseTone * _TAMIndirectContribution);
    return lerp(tone, tone * tamHatchData.occlusion, _TAMAOContribution);
}
...
struct Varyings
{
    ...
    // Explicit mesh-attached projection data. It is independent of CupOHappiness UV modes.
...
#include "Includes/Toon SurfaceData.hlsl"
...
//  be guarded as well - otherwise DEBUG_DISPLAY breaks on materials without any texture.
    ...
    half3 legacyRimLighting = 0.0h;
        ...
        legacyRimLighting = pow(rim, power) * _RimColor.rgb * _RimColor.a;
    #endif
...
//  Apply lighting
    CupOHappinessHatchData tamHatchData;
    half4 color = CupOHappinessURPToonFragmentPBR(
        inputData,
    ...
    #if defined(_TAM_ENABLED)
        half tamTone = CupOHappinessTAMResolveTone(tamHatchData);
        half tamInkAlpha = 1.0h;
        #if defined(_ALPHAPREMULTIPLY_ON)
            tamInkAlpha = color.a;
        #endif
        color.rgb = CupOHappinessTAMComposeShadowFormInk(
            color.rgb, input.positionOS, input.normalOS, tamTone, tamInkAlpha);
        color.rgb = CupOHappinessTAMComposeLightInk(
            color.rgb, input.positionOS, input.normalOS,
            tamHatchData.directDiffuseTone, tamHatchData.highlightDomain,
            tamHatchData.directLightTint, tamInkAlpha);
        color.rgb = CupOHappinessTAMComposeHighlightPunchOut(
            color.rgb, input.positionOS, input.normalOS,
            tamHatchData.highlightDomain, tamHatchData.highlightTint);
        color.rgb = CupOHappinessTAMDiagnosticColor(
            color.rgb, input.positionOS, input.normalOS,
            tamHatchData.directDiffuseTone, tamTone, tamHatchData.highlightDomain);
    #endif
...
//  Fix alpha - matches: color.a = OutputAlpha(color.a, IsSurfaceTypeTransparent(_Surface));
    color.a = _CupSurface == 1 ? color.a : 1;
    outColor = color;
