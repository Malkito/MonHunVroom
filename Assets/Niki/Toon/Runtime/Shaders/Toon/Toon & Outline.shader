Shader "CupOHappiness/Toon & Outline"
{
    Properties
    {
        [Space(8)]
        ...
        _CupSurface                 ("Surface Type", Float) = 0.0
        [Enum(Alpha,0,Premultiply,1,Additive,2,Multiply,3)]
        _CupBlend                   ("     Blending", Float) = 0.0
        [HideInInspector]
        ...
        [Space(8)]
        [Toggle(_TAM_ENABLED)]
        _TAMEnabled                         ("Enable TAM Hatching", Float) = 0.0
        _TAMIndirectContribution             ("Indirect Light Contribution", Range(0.0, 1.0)) = 0.0
        _TAMAOContribution                   ("Occlusion Contribution", Range(0.0, 1.0)) = 0.0

        [Space(8)]
        [NoScaleOffset]
        _TAMShadowFormArray                  ("TAM Asset (Light to Dark)", 2DArray) = "" {}
        _TAMShadowFormProjectionScale        ("Projection Scale", Range(0.01, 100.0)) = 1.0
        _TAMShadowFormToneBias                ("Tone Bias", Range(-1.0, 1.0)) = 0.0
        [HDR] _TAMShadowFormInkColor          ("Ink Color", Color) = (0,0,0,1)
        _TAMShadowFormInkOpacity              ("Ink Opacity", Range(0.0, 1.0)) = 1.0
        _TAMShadowFormRange                   ("Range (0 Off → 1 Full Shadow Domain)", Range(0.0, 1.0)) = 0.0
        _TAMShadowFormOpacity                 ("Persistent Form Opacity", Range(0.0, 1.0)) = 0.0

        [Space(8)]
        [NoScaleOffset]
        _TAMLightArray                       ("TAM Asset (Light to Dark)", 2DArray) = "" {}
        _TAMLightProjectionScale             ("Projection Scale", Range(0.01, 100.0)) = 1.0
        _TAMLightToneBias                    ("Tone Bias", Range(-1.0, 1.0)) = 0.0
        [HDR] _TAMLightInkColor              ("Ink Color", Color) = (0,0,0,1)
        _TAMLightInkOpacity                  ("Ink Opacity", Range(0.0, 1.0)) = 1.0
        _TAMLightRange                       ("Range (0 Off → 1 Full Light Domain)", Range(0.0, 1.0)) = 0.0

        [Space(8)]
        [NoScaleOffset]
        _TAMHighlightPunchOutArray           ("TAM Asset (Light to Dark)", 2DArray) = "" {}
        _TAMHighlightPunchOutProjectionScale ("Projection Scale", Range(0.01, 100.0)) = 1.0
        _TAMHighlightPunchOutToneBias        ("Tone Bias", Range(-1.0, 1.0)) = 0.0
        _TAMHighlightPunchOutOpacity         ("Opacity", Range(0.0, 1.0)) = 0.0
        _TAMLightHighlightTransparency       ("Light Hatching Transparency in Highlights", Range(0.0, 1.0)) = 0.0

        [Space(8)]
        [Enum(Final Composition,0,Direct Light Shape,1,Direct Shadow Shape,2,Specular Rim Shape,3,Shadow Form Coverage,4,Light Coverage,5,Highlight Punch Out Coverage,6)]
        _TAMDebugView                  ("View", Float) = 0.0

        
    //  CupOHappiness URP standards 
        [Space(8)]
        [Toggle(_RIMLIGHTING)]
        ...
        [HideInInspector]
        _CupSurface                 ("__CupSurface", Float) = 0.0
...
//  Unity 6: Outlines now are rendered using a Renderer Feature
            ...
            Name "CupOHappinessOutline"
            Tags
            {
                "LightMode" = "CupOHappinessOutline"
            }
            ...
            // URP 17 provides fog functions through Core.hlsl -> ShaderVariablesFunctions.hlsl.
            ...
            #include "Includes/Toon Inputs.hlsl"
            #include "Includes/Toon Outline Pass.hlsl"
        ...
        Pass
        {
            ...
            HLSLPROGRAM
            // Texture2DArray sampling and runtime array-dimension queries require SM 4.5.
            #pragma target 4.5
            ...
            #pragma shader_feature_local_fragment _TAM_ENABLED
        ...
        //  Include base inputs and all other needed "base" includes
            #include "Includes/Toon Inputs.hlsl"
            #include "../Includes/TAM Triplanar.hlsl"
            #include "Includes/Toon ForwardLit Pass.hlsl"
    ...
    //  Shadows -----------------------------------------------------
        ...
        //  Include base inputs and all other needed "base" includes
            #include "Includes/Toon Inputs.hlsl"
            #include "Includes/Toon ShadowCaster Pass.hlsl"
    ...
    //  GBuffer Pass - minimalized which only outputs depth and writes into the normal buffers  
        ...
        //  As we do not store the alpha mask with the base map we have to use custom functions 
            ...
            #include "Includes/Toon Inputs.hlsl"
            // URP 17 replacement for the removed GBufferOutput.hlsl.
    ...
    //  Depth -----------------------------------------------------
        Pass
        {
            ...
            #include "Includes/Toon Inputs.hlsl"
            #include "Includes/Toon DepthOnly Pass.hlsl"
    ...
    //  Depth Normals -----------------------------------------------------
        Pass
        {
            ...
            #include "Includes/Toon Inputs.hlsl"
            #include "Includes/Toon DepthNormal Pass.hlsl"
            ENDHLSL
        }    
    ...
    //  Meta -----------------------------------------------------
        ...
        //  First include all our custom stuff
            #include "Includes/Toon Inputs.hlsl"
            #include "Includes/Toon Meta Pass.hlsl"
...
#pragma shader_feature_local _OUTLINEINSCREENSPACE
            ...
            #include "Includes/Toon Inputs.hlsl"
#include_with_pragmas "Includes/Toon Outline MV.hlsl"
            ENDHLSL
        }
...
#include_with_pragmas "Includes/Toon Outline MV.hlsl"
    ...
    CustomEditor "CupOHappiness.Toon.Editor.CupOHappinessToonShaderGUI"
