...
#ifndef CUPOFHAPPINESS_TOON_LIGHTING_INCLUDED
#define CUPOFHAPPINESS_TOON_LIGHTING_INCLUDED
...
//  During the different Unity 6 betas it was or was not defined... so we can't use it.
    ...
    SAMPLER(cup_point_clamp_sampler);
    SAMPLER(cup_linear_clamp_sampler);
...
//     float exponent = RoughnessToBlinnPhongSpecularExponent_CupOHappiness(1 - smoothness);
...
// Compact data exported at the CupOHappiness forward-lighting seam. Values are captured
// after CupOHappiness's toon direct-light response, but before final RGB composition.
// They intentionally omit emission and rim, which retain their CupOHappiness-only roles.
struct CupOHappinessHatchData
{
    half directDiffuseTone;
    half indirectDiffuseTone;
    half occlusion;
    half specularTone;
    half highlightDomain;
    half3 highlightTint;
    half3 shadowSurfaceTint;
    half3 surfaceTint;
    half3 directLightTint;
    half3 specularTint;
};

half4 CupOHappinessURPToonFragmentPBR(InputData inputData,
    ...
    half smoothness, half occlusion, half3 emission, half3 legacyRimLighting, half alpha,
    float4 positionCS,
    out CupOHappinessHatchData tamHatchData
)
{
    // Debug-display overrides return before CupOHappiness has evaluated the light loop.
    // Use a fully lit direct response so TAM cannot alter CupOHappiness diagnostics.
    tamHatchData.directDiffuseTone = 1.0h;
    tamHatchData.indirectDiffuseTone = 0.0h;
    tamHatchData.occlusion = 1.0h;
    tamHatchData.specularTone = 0.0h;
    tamHatchData.highlightDomain = 0.0h;
    tamHatchData.highlightTint = 0.0h;
    tamHatchData.shadowSurfaceTint = albedo;
    tamHatchData.surfaceTint = albedo;
    tamHatchData.directLightTint = 1.0h;
    tamHatchData.specularTint = 0.0h;
...
//  Rim - do it here in order to be able to add it to the debug view
    ...
    //  Stabilize rim
        ...
        surfaceData.emission = emission + legacyRimLighting;
...
//  Global Illumination
    ...
    // Keep baked/merged indirect light and occlusion independently available
    // so material controls can opt in without sampling final composed RGB.
    tamHatchData.indirectDiffuseTone = saturate(Luminance(inputData.bakedGI));
    tamHatchData.occlusion = saturate(occlusion);
    ...
    { 
        ...
        //  IMPORTANT: no saturate on the 2nd param: NdotL - 0.01. 0.01 is eyballed.
            NdotL = (quantizedNdotL + aaStep(saturate(quantizedNdotL * oneOverSteps), NdotL - 0.01h, diffuseFalloff )) * oneOverSteps;
                ...
                NdotL = SAMPLE_TEXTURE2D(_GradientMap, cup_linear_clamp_sampler, float2 (NdotL, 0.5f)).r;
            #else
                half NdotL0 = SAMPLE_TEXTURE2D(_GradientMap, cup_point_clamp_sampler, float2 (NdotL, 0.5f)).r;
                half NdotL1 = SAMPLE_TEXTURE2D(_GradientMap, cup_point_clamp_sampler, float2 (NdotL + fwidth(NdotL) * _GradientMap_TexelSize.x, 0.5f)).r;
...
//  Decals Part 2
    ...
    half3 toonRimContribution = 0.0h;
    #if defined(_TOONRIM)
        toonRimContribution = rimLighting * lerp(1.0h, lightIntensity, toonRimAttenuation);
    #endif
    ...
    //  rim lighting 
            ...
            + toonRimContribution
        #endif
        // Legacy rim is owned by CupOHappiness as well, but is supplied by the
        // ForwardLit surface path because CupOHappiness exposes it as emission.
        + legacyRimLighting
...
#endif
    Lighting += emission;
    // Includes CupOHappiness's post-step/post-ramp response, shadow attenuation, and
    // all supported per-pixel additional lights. Specular is exported for the
    // highlight layer but never participates in baseline TAM density.
    tamHatchData.directDiffuseTone = saturate(lightIntensity);
    half3 specularContribution = specularLighting * lightIntensity * lightColor;
    tamHatchData.specularTone = saturate(Luminance(specularContribution));
    tamHatchData.highlightDomain = saturate(
        Luminance(specularContribution) + Luminance(toonRimContribution) + Luminance(legacyRimLighting));
    tamHatchData.highlightTint = specularContribution + toonRimContribution + legacyRimLighting;
    tamHatchData.shadowSurfaceTint = shadedAlbedo;
    tamHatchData.surfaceTint = litAlbedo;
    tamHatchData.directLightTint = lightColor;
    tamHatchData.specularTint = specularContribution;
